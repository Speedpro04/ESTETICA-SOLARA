-- =============================================================================
-- Solara Estética — cobrança da assinatura pelo Stripe
-- =============================================================================
-- Espelha supabase/schema_v2/19_stripe.sql (o schema_v2 é a fonte de verdade;
-- esta é a versão aplicável em banco já existente). Idempotente: pode rodar de
-- novo sem estragar nada.
--
-- Reverte a decisão de cobrar direto pelo Banco Cora. Motivo: o Cora não expõe
-- cartão recorrente nem Pix Automático por API, então a mensalidade de R$497
-- dependeria de a clínica pagar um boleto por mês — churn involuntário virando
-- problema de produto. O Stripe cobra sozinho.
--
-- O trial de 10 dias continua sendo local (register_clinic cria 'trialing' sem
-- cartão). O Stripe só entra quando a clínica decide pagar.
--
-- A conta do Stripe é compartilhada com outro produto do mesmo CNPJ, e o
-- endpoint da Solara recebe TODOS os eventos da conta. Por isso tudo aqui
-- ignora em silêncio o que não reconhece, em vez de adivinhar a linha próxima.
--
-- Os stripe_price_id de producao ja vao preenchidos na secao 6. Sem eles o backend
-- recusa abrir o checkout (503), de propósito.
-- =============================================================================
